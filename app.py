...
# 응답시간 추이 그래프 기본 표시 페이지
DEFAULT_TREND_PAGES = ["홈(루트)", "메인", "경기일정", "일반예매", "공지사항", "팬서비스"]
...
def render_response_trend():
    ...
    default = [n for n in DEFAULT_TREND_PAGES if n in names]
    picked = st.multiselect("페이지 선택", names, default=default)
    if not picked:
        return
    frames = []
    for name in picked:
